...
FunctionState::FunctionState(char *name, std::function<void()> on_enter,
                             std::function<void()> on_state,
                             std::function<void()> on_exit)
    : on_enter{std::move(on_enter)},
      on_state{std::move(on_state)},
      on_exit{std::move(on_exit)}, name(name)
{
}
...
void FunctionFsm::run_machine()
{
  ...
  if (!m_initialized)
  {
    ...
    { // if nullptr (see [func.wrap.func] paragraph 8 in standard)
...
void FunctionFsm::make_transition(Transition *transition)
{
  ...
  { // if not nullptr (see [func.wrap.func] paragraph 8 in standard)
